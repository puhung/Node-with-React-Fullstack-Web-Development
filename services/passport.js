...
//any time we touch database, that is an asynchronous interaction, so it always returns a promise to be deal with.
passport.use(new GoogleStrategy({
    ...
    async (accessToken, refreshToken, profile, done) => {
      ...
      const existingUser = await User.findOne({ googleId: profile.id}) //this query returns a promise, which is a tool that we use for JS for handling asynchronous code
      if (existingUser){
        //if this existingUser exists , we have a record with profile ID

        return done(null, existingUser); // provide 2 arguments to this function
